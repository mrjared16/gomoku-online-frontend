...
const useStyles = makeStyles((theme) => ({
  root: { backgroundColor: 'var(--color-background)' },
  table: {
...
function History() {
  ...
  const renderElo = (newRank, oldRank) => {
    if (newRank > oldRank) {
      return (
        <span className={classes.win}>{`${newRank} (+${
          newRank - oldRank
        })`}</span>
      );
    }
    if (oldRank > newRank) {
      return (
        <span className={classes.lose}>{`${newRank} (-${
          oldRank - newRank
        })`}</span>
      );
    }
    if (oldRank === newRank) {
      return <span className={classes.tie}>{`${newRank}`}</span>;
    }
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      cellClassName: (param) =>
        param.value === 0
          ? `custom-cell__center ${classes.sideXColumn}`
          : `custom-cell__center ${classes.sideOColumn}`,
    ...
    },
    {
      ...
      renderCell: (param) => (
        <span>
          {renderResult(param.value?.gameResult, param.value?.playerSide)}
        </span>
      ),
      width: 120,
    },
    ...
    },
    {
      ...
      cellClassName: 'custom-cell__center',
      renderCell: (param) => (
        <span>{param?.value && moment.unix(param.value).format('mm:ss')}</span>
      ),
      width: 150,
    },
    {
    ...
    },
    {
      ...
      renderCell: (param) => (
        <Button
          ...
          disabled={Boolean(currentRoomID)}
        >
          Watch
        </Button>
      ),
