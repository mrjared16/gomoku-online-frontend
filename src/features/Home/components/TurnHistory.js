  ...
  return (
    <div className={classes.root}>
      ...
      <div className={classes.body}>
        <List>
          {list &&
            list.map(({ position, value }, index) => (
              <ListItem
                key={index}
                button
                selected={selectedIndex === index}
                onClick={() => handleChangeMoveIndex(index)}
              ...
              >
                ...
                <ListItemText>
                  {value === 0 ? (
                    <span className={classes.xPlayer}>X</span>
                  ) : (
                    <span className={classes.oPlayer}>O</span>
                  )}
                  {`(${formatPosition(position).x}, ${
                    formatPosition(position).y
                  })`}
                </ListItemText>
              </ListItem>
            ))}
